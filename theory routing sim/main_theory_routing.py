...
sigma = 468 #enter your current students, however it will recalculate this to a higher number if you are underpushed
time = 0


main_theory_values = [668,710,671,853,1000,1412,626,537] # t1,t2,t3,t4,t5,t6,t7,t8

cts = 3143 # enter current total cts tau
...
def ct_tau_gain(cts,time):
    if time < 48264:  #go to the end of ct_data rounded down to nearest time in days and multiply by 24 replace this number here so it doesnt go out of range
...
def calculate_rates(main_theory_rates, sigma,cts,time):
    ...
    if (tau_current < 4550):
        ...
        sigma = math.floor(345 + (tau_current - 7070) / 20.27)
    elif (tau_current >= 10110):
    ...
    with open(r"to do", "a") as file: #set absolute path of "main_theory_routing.txt"
        file.write(f"{sigma} ")
        for value in main_theory_values:
            file.write(f"{value:.3f} ")
        file.write(f"{sum(main_theory_values):.3f} {cts} {round(time / 24, 3)}\n")
...
def cash_in(main_theory_values, main_theory_rates, time, sigma,cts):
    ...
    #while main_theory_values[3] < 836: #if trying to minimize time turn disable line above and turn on and set t4 e1 less than greedy value of t4
        calculate_rates(main_theory_rates, sigma,cts,time)
        overpush_rates = [main_theory_rates[0],
                              (main_theory_rates[1]),
                              main_theory_rates[2],
...
def greedy_strat(main_theory_values, main_theory_rates, time, sigma,cts):
    while sum(main_theory_values)+cts < 10110: # set a stop condition
    #while time < 111:
    #while main_theory_values[5] < 2000:
    #while sum(main_theory_values)+cts < 11336:
        calculate_rates(main_theory_rates, sigma,cts,time)
        fastest = main_theory_rates.index(max(main_theory_rates))
        pub_time = pub_length[fastest] / main_theory_rates[fastest]
        main_theory_values[fastest] += pub_length[fastest]
...
def overpush_strat(main_theory_values, main_theory_rates, time, sigma,cts):
    #while sum(main_theory_values)+cts< 4300:
...
def overpush_strat_2(main_theory_values, main_theory_rates, time, sigma,cts):
    while (main_theory_values[1] < 690) :  # caps t2 otherwise it WILL push t2 to 780-800+ (if trying to minimize time set this t2 to e1 less than greedy t2)
        calculate_rates(main_theory_rates, sigma,cts,time)
...
# Example usage
file_path = r"to do" #set absolute path of ct_tau_lookup_table.txt
